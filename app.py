...
st.markdown('''
    ...
    @media (prefers-color-scheme: dark) {
      ...
      .feedback-card, .feedback-card * { color: #ffffff !important; }
      .question-card, .question-card * { color: #ffffff !important; }
...
@st.cache_resource
def get_agent():
    """Get the appropriate LLM for evaluation and conversation (without react agent wrapper)."""
    if LLM_PROVIDER == "gemini" and GEMINI_API_KEY:
        llm = ChatGoogleGenerativeAI(
            model=DEFAULT_MODEL["gemini"],
            temperature=0,
...
def load_assignment(sid: str) -> Optional[dict[str, Any]]:
    ...
    # Check if this is a debate assignment (must end with _da)
    if not aid_key.lower().endswith('_da'):
        st.error('This is not a debate assignment. Debate assignments must end with "_da".')
        return None
    ...
    # Mark as debate mode
    print(f"[DEBUG] Loading DEBATE assignment: {aid_key}")
...
def is_valid_debate_response(response_text: str) -> bool:
    """Validate that a Devil's Advocate response is substantial and not malformed."""
    if not isinstance(response_text, str):
        return False
    
    text = response_text.strip()
    
    # Check minimum length (DA responses should be substantial)
    if len(text) < 50:
        return False
    
    # Check it's not just an error message
    error_indicators = ['error', 'failed', 'unable to', 'malformed', 'could not']
    if any(indicator in text.lower() for indicator in error_indicators):
        if len(text) < 200:  # Short error messages are bad
            return False
...
def run_grading_streaming(exec_id: str, sid: str, aid: str, answers: Dict[str, str]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Build debate history (DA first, then User per exchange)
        ...
        try:
            ...
            for answer_rec in all_answers:
                ...
                if exec_id_key and user_arg:
                    exchanges.append({'exec_id': exec_id_key, 'timestamp': timestamp, 'user': user_arg, 'da': ''})
            ...
            print(f"[WARN] Failed to build DA debate history: {e}")
        debate_history = "\n\n".join(debate_history_parts) if debate_history_parts else "No prior debate history."
        
        # Get judge history
        judge_history = st.session_state.get('judge_history', '')
        
        # Build structured prompt with <data>, <orchestration>, <output_format>, <instructions>
        orchestration_text = """Core response behavior (format and style):
- Output ONLY a single short paragraph (no lists, no headers, no JSON).
- Speak like a real person in a live debate: natural, conversational, varied sentence length.
- Keep it tight: 1–2 strong points total, each 1–2 sentences max.
- Start by reacting to the user's latest point, then build YOUR own counter-argument.
- Do not dump everything at once; save arguments for later rounds.
- Do not write an essay or use formal academic phrasing.
- Include at least one unique supporting claim/evidence that advances your counter-position.
- Aim to be persuasive to a neutral judge."""
        
        output_format_text = """A single short paragraph (1–5 sentences) that:
- Reacts to the user's latest point,
- Presents 1–2 focused counter-points in natural language,
- Contains no bullets, no numbering, no headers, no JSON."""
        
        devils_advocate_message = f"""<data>
Execution ID: {exec_id}
Student ID: {sid}
Assignment ID: {aid}
Debate Topic: {debate_topic}

Current User Argument (respond to this):
{user_current_argument}

Full Debate History (Devil's Advocate first, then User for each exchange):
{debate_history}

Prior Judge Interactions:
{judge_history if judge_history else "None"}
</data>
...
def initialize_judge_evidence_lists(exec_id: str, sid: str, aid: str, debate_topic: str) -> None:
    """Generate and cache 10 PRO and 10 COUNTER evidence points for the judge once per session."""
    try:
        # Skip if already initialized
        if st.session_state.get('judge_evidence_initialized'):
            print("[JUDGE INIT] Evidence lists already initialized, skipping")
            return
        if assignment_memory.current_state and assignment_memory.current_state.get('session_metadata', {}).get('judge_evidence'):
            st.session_state['judge_evidence_initialized'] = True
            print("[JUDGE INIT] Evidence lists found in memory, skipping")
            return

        print(f"[JUDGE INIT] Generating evidence lists for topic: {debate_topic[:100]}...")
        
        # Build init prompt for judge
        init_prompt = f"""You are the Judge in a debate simulation. Before the debate proceeds, precompute evidence reference lists.

Task: Create two lists of 10 concise evidence points each for the debate topic below.
- One list for the PRO position
- One list for the COUNTER position

Debate Topic: {debate_topic}

Output ONLY valid JSON (no extra text) exactly in this format:
{{
  "pro_points": ["point 1", "point 2", ... (10 items total)],
  "counter_points": ["point 1", "point 2", ... (10 items total)]
}}"""

        # Use judge agent with stable thread id
        judge_thread_id = f"judge_{sid}_{aid}"
        config = {"configurable": {"thread_id": judge_thread_id}}

        # Invoke judge agent for single-shot response
        result_text = ""
        try:
            result = judge_agent.invoke({"messages": [HumanMessage(content=init_prompt)]}, config=config)
            if "messages" in result:
                for msg in result["messages"]:
                    if isinstance(msg, AIMessage) and msg.content:
                        result_text += msg.content
        except Exception as e:
            print(f"[WARNING] Judge evidence init via agent failed: {e}")
            result_text = ""

        # Parse JSON
        pro_points, counter_points = [], []
        if result_text:
            try:
                import re
                ...
                pro_points = data.get('pro_points') or []
                counter_points = data.get('counter_points') or []
            except Exception as e:
                print(f"[WARNING] Could not parse judge evidence JSON: {e}")

        # Sanitize and trim to max 10 items each
        ...
        pro_points = _sanitize_list(pro_points)
        counter_points = _sanitize_list(counter_points)
        
        evidence_payload = {"pro": pro_points, "counter": counter_points}

        # Store in memory
        if assignment_memory.current_state:
            if 'session_metadata' not in assignment_memory.current_state:
                assignment_memory.current_state['session_metadata'] = {}
            assignment_memory.current_state['session_metadata']['judge_evidence'] = evidence_payload

        # Store in session state
        st.session_state['judge_evidence'] = evidence_payload
        st.session_state['judge_evidence_initialized'] = True
        print(f"[JUDGE INIT] Evidence lists initialized (PRO={len(pro_points)}, COUNTER={len(counter_points)})")
    except Exception as e:
        print(f"[ERROR] initialize_judge_evidence_lists failed: {e}")
...
def run_judge(exec_id: str, sid: str, aid: str) -> Dict[str, Any]:
    ...
    try:
        # Wait for judge evidence initialization if still running
        if not st.session_state.get('judge_evidence_initialized') and st.session_state.get('judge_evidence_thread'):
            thread = st.session_state.get('judge_evidence_thread')
            if thread and thread.is_alive():
                print("[JUDGE INIT] Waiting for evidence generation to complete...")
                with st.spinner("⚖️ Judge is preparing evidence lists..."):
                    thread.join(timeout=30)  # Wait up to 30 seconds
                print("[JUDGE INIT] Evidence generation complete or timed out")
        ...
        try:
            ...
            # Sort by timestamp and build history string (DA first, then User)
            exchanges.sort(key=lambda x: x.get('timestamp', ''))
            for exchange in exchanges:
                if exchange['da']:
                    debate_history_parts.append(f"Devil's Advocate: {exchange['da']}")
        ...
        # Build structured judge prompt with <data>, <orchestration>, <output_format>, <instructions>
        orchestration_text = """Core judging behavior (format and decision rules):
- Output ONLY valid JSON matching <output_format>. No extra prose.
- Use conversational, concise language in 'reasoning' (2–3 sentences max).
- Decision protocol:
  1) Maintain two mental lists of 10 evidence points (PRO and COUNTER) for the topic.
  2) For each side, count how many presented, well-supported claims match (or are equivalent to) points in those lists.
  3) If either side reaches 3+ well-supported claims: set decision_made=true and winner accordingly.
  4) If neither reaches 3: set decision_made=false and ask ONE specific follow_up_question; set question_for.
  5) If both reach 3+ and it's a tie: set decision_made=false and ask a tiebreaker about the most critical unresolved issue.
- If decision_made=true: set follow_up_question=null and question_for=null.
- Questions must end with '?', be specific, and target the exact evidence gap.
- Avoid robotic phrasing; reasoning should sound like a thoughtful human judge."""
        
        output_format_text = """{
  "decision_made": true/false,
  "winner": "user" or "devils_advocate" or null,
  "reasoning": "Your conversational explanation",
  "follow_up_question": "Your specific question?" or null,
  "question_for": "user" or "devils_advocate" or null
}"""
        
        # Get cached judge evidence if available
        judge_evidence = st.session_state.get('judge_evidence') or (assignment_memory.current_state or {}).get('session_metadata', {}).get('judge_evidence')
        evidence_text = ""
        if judge_evidence:
            pro = judge_evidence.get('pro', [])
            counter = judge_evidence.get('counter', [])
            if pro and counter:
                evidence_text = f"\n\nPre-computed Evidence Lists (use these as your reference):\nPRO Points: {json.dumps(pro)}\nCOUNTER Points: {json.dumps(counter)}"
        
        judge_prompt = f"""<data>
Execution ID: {exec_id}
Student ID: {sid}
Assignment ID: {aid}
Debate Topic: {debate_topic}{evidence_text}
...
def main() -> None:
    ...
    with st.container():
        ...
        # Initialize judge evidence lists in background (non-blocking)
        if not st.session_state.get('judge_evidence_initialized') and not st.session_state.get('judge_evidence_thread_started'):
            try:
                debate_topic = qrec.get('Question1', '')
                thread = threading.Thread(
                    target=initialize_judge_evidence_lists,
                    args=(exec_id, sid, aid, debate_topic),
                    daemon=True
                )
                thread.start()
                st.session_state['judge_evidence_thread'] = thread
                st.session_state['judge_evidence_thread_started'] = True
                print("[JUDGE INIT] Started background thread for evidence generation")
            except Exception as e:
                print(f"[WARN] Failed to start judge evidence thread: {e}")
        ...
        if fb and submitted:
            ...
            if text:
                st.markdown(
                    f"""
                    <div style='background:rgba(255,90,90,0.15); color:inherit; border-radius:8px; padding:1.2rem; margin-bottom:0.7rem; margin-top:0.5rem; font-size:1.08rem; border-left: 4px solid rgba(255,90,90,0.6);'>
        ...
        if st.session_state.get('judge_mode', False):
            ...
            if st.session_state.get('calling_judge', False):
                ...
                # Write judge response to conversations sheet
                if judge_result:
                    winner_value = None
                    ...
                    judge_conv_data = {
                        ...
                        "student_id": sid,
                        "user_msg": None,
                        "agent_msg": judge_result.get('reasoning', judge_result.get('full_response', '')),
                        "timestamp": datetime.datetime.now().isoformat(sep=' ', timespec='seconds'),
                        "winner": winner_value
                    }
                    background_writer.write_async('conversations', judge_conv_data)
            ...
            if judge_result:
                ...
                st.markdown(
                    f"""
                    <div style='background:rgba(255,215,0,0.15); color:inherit; border-radius:8px; padding:1.5rem; margin-bottom:1rem; font-size:1.08rem; border-left: 4px solid rgba(255,215,0,0.7);'>
                ...
                if judge_result.get('decision_made'):
                    ...
                    if winner == 'user':
                        ...
                        # Mark assignment as completed when user wins
                        current_completed = sa.get('completed', 'FALSE').upper()
                        if current_completed != 'TRUE':
                            try:
                                sheets.update_completed_status(sid, aid, 'TRUE')
                                print(f"[COMPLETION] Marked debate assignment {aid} as completed for student {sid}")
                            except Exception as e:
                    ...
                    if question_for == 'user':
                        ...
                        if st.button('Submit Response to Judge', key='submit_judge_user_btn', use_container_width=True, type='primary'):
                            if not user_response.strip():
                                ...
                                # Write user response to conversations sheet
                                user_to_judge_data = {
                                    "execution_id": exec_id,
                                    "assignment_id": aid,
                                    "student_id": sid,
                                ...
                                background_writer.write_async('conversations', user_to_judge_data)
                        ...
                        with st.spinner("😈 Devil's Advocate is responding to the Judge..."):
                            ...
                            # Write DA response to conversations sheet (treat as user_msg)
                            da_to_judge_data = {
                                "execution_id": exec_id,
                                "assignment_id": aid,
                                "student_id": sid,
                            ...
                            background_writer.write_async('conversations', da_to_judge_data)
